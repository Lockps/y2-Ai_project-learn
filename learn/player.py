...
class Player:
    ...
    def pipe_collision(self):
        for p in config.pipes:
            return pygame.Rect.colliderect(
                self.rect, p.top_rect
            ) or pygame.Rect.colliderect(self.rect, p.bottom_rect)
    ...
    def look(self):
        if config.pipes:
            # Line to top pipes
            self.vision[0] = (
                max(0, self.rect.center[1] - self.closest_pipes().top_rect.bottom) / 500
            )
            pygame.draw.line(
                config.window,
                self.color,
            ...
            # Line to mid pipes
            self.vision[1] = max(0, self.closest_pipes().x - self.rect.center[0]) / 500
            pygame.draw.line(
                config.window,
                self.color,
                self.rect.center,
            ...
            # Line to top pipes
            self.vision[2] = (
                max(0, self.closest_pipes().bottom_rect.top - self.rect.center[1]) / 500
            )
            pygame.draw.line(
                config.window,
                self.color,
